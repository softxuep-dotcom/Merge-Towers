...
export class TowerUpgradeOverlay {
  ...
  card({ x, y, w, h, color, icon, name, description, badge, index, onPick, compact = false, disabled = false }) {
    const s = this.scene;
    ...
    if (disabled) {
      c.setAlpha(0.42);
      bg.setFillStyle(0x12181b, 0.94).setStrokeStyle(2, 0x596268, 0.5);
      badgeText.setColor('#8d999f');
    } else {
  ...
  bindKeys(items, onPick, onCancel) {
    this.keyHandler = event => {
      ...
      if (index >= 0 && index < items.length && !items[index]?.disabled) onPick(items[index]);
    };
  ...
  showChoices(tower, choices, onPick, onCancel, options = {}) {
    ...
    this.begin(`${elem.cn}  Lv${tower.lv} → Lv${tower.lv + 1}`, options.subtitle || t('upgradeCards.lockedChoice'));
    if (portrait) {
      choices.forEach((choice, i) => this.card({
        ...
        badge: choice.badge, disabled: choice.disabled, onPick: () => onPick(choice),
      }));
    } else {
